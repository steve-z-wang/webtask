...
def build_planner_prompt() -> str:
    ...
    qa = (
        ...
        .add("Focus on WHAT needs to happen, not HOW. Define clear, verifiable goals that the Worker can understand and the Verifier can check.")
        .add()
        .add("**How many subtasks should you plan at once?**")
        .add("Plan ONE subtask at a time. Wait for the current subtask to complete before planning the next one.")
        .add()
        .add("**What makes a good subtask goal?**")
        .add("Goals should be specific, achievable, and verifiable. Example: 'Add 2 screws to cart' is better than 'Get screws'.")
    )
    ...
    response_format = (
        ...
        .add()
        .add('Example: {"observation": "No subtasks yet", "thinking": "Need to add first item", "tool_calls": [{"description": "Started subtask to add screws", "tool": "start_subtask", "parameters": {"goal": "add 2 screws"}}]}')
    )
